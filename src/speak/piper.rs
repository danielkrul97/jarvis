//! Lokální TTS záloha: piper (neuronová syntéza na CPU, hlas
//! cs_CZ-jirka-medium). Subprocess pattern jako parec/ffplay; nic neopouští
//! stroj a syntéza je zdarma — proto slouží jako fallback, když ElevenLabs
//! selže (kvóta, síť, klíč), nebo jako jediný engine (`speak.engine="piper"`).
...
/// "cs_CZ-jirka-medium" → "cs/cs_CZ/jirka/medium" (adresář v repu hlasů).
...
/// Stáhne hlas (.onnx + .onnx.json) z rhasspy/piper-voices do models_dir.
...
/// Syntéza textu do WAV souboru `out`. Text se skládá na jeden řádek —
/// piper bere každý řádek stdin jako samostatnou promluvu.
pub fn synthesize(paths: &Paths, s: &SpeakCfg, text: &str, out: &Path) -> Result<()> {
    ...
    // ElevenLabs speed (vyšší = rychlejší) ↔ piper length-scale (vyšší = pomalejší)
    ...
    // drop stdin (konec statementu výš) = EOF → piper syntetizuje a končí
...
mod tests {
    ...
    fn voice_repo_dir_shapes() {
        ...
        // víceslovné jméno hlasu drží pohromadě
